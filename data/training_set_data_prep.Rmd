...
# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
	...
	for (i in 2:(length(homes)))
	{
		if(class(homes[,i]) == "factor")
		{
			...
			p <- ggplot(homes, aes(x = homes[,i], fill = homes[,i])) + 
				geom_bar() + labs(title = plot_title)
			print(p)

			p <- ggplot(homes, aes(x = homes[,i], y = log(saleprice), fill = homes[,i])) + 
...
``` {r anova for foundation features, include = TRUE, message = FALSE}

# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
# ...	make some box plots for numeric variables vs. foundation type for LDA model
# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

	pdf ("homes_train_foundation_boxplots.pdf", width = 10, height = 7)

	par (mfrow = c (2, 2))
	for (i in 2:(length(homes)))
	{
		if(class(homes[,i]) == "numeric")
		{
			plot_title <- names(homes[i])
			
			p <- ggplot(homes, aes(x = homes[,i], fill = foundation)) +
    				geom_histogram() + labs(title = plot_title)
			print(p)			
			
			p <- ggplot(homes, aes(x = foundation, y = homes[,i], fill = foundation)) + 
				geom_boxplot() + labs(title = plot_title)
			print(p)
		}
	}	
...
```
``` {r visual retention for LDA on foundation, include = TRUE, message = FALSE}

# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
# ...	Columns to retain - based on visual inspection of histograms / boxplots
# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

	homes$log_saleprice <- log(homes$saleprice)

	homes_lda_keep <- subset(homes,
			select = c(
				log_saleprice,
				foundation,
				lotfrontage, # ... wood
...
# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
# ..	- impute discretes are left out ... not that many imputed values
# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
	...
	setwd(home_dir)
	setwd(lda_dir)
	write.csv (homes_lda_keep, file = "lda_training_set_cleaned.csv", row.names = FALSE)
	setwd(home_dir)
