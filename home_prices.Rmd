...
# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
	...
	for (i in 1 : (length(homes_subset)))
	{
			...
			print(sprintf(" ... %3d : %20s | r^2 = %8.3f | p-value = %12.4e",
						  i, names(homes_subset[i]), summary(fit)$r.squared, summary(fit)$coefficients[,4][2] ))
			
			next_row <- data.frame(row = i, feature = names(homes_subset[i]), r_squared = summary(fit)$r.squared)
